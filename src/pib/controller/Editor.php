...
class Editor extends Controller{
    ...
    public function actionEncodeAjax($id){
        ...
        if($creation instanceof Creation){
            ...
            $shell .= 'ffmpeg -i ' . $directory . '/web/pib/file/' . $creation->video->path . ' -i ' . $directory . $creation->music->path . ' -filter_complex "[0:a][1:a]amerge=inputs=2[a]" -map 0:v -map "[a]" -c:v copy -c:a libvorbis -ac 2 -shortest ' . $directory . $directoryCreation . $id . '.1.temp.mp4'."\n";
